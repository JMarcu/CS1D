...
class Edge
{
	public:
	    ...
	    *******************************************/
	    //Default constructor
		Edge()
			{firstVertex = NULL; secondVertex = NULL; weight = 0.0;}
...
class Graph
{
	public:
		...
		*******************************************/
		...
		const Vertex<ElemType>* GetVertex(const ElemType& key) const;

		//Returns the weight of the edge between the two. Returns -1 if
		//no edge found.
		float GetDistance(const ElemType& start, const ElemType& stop) const;
		...
		*******************************************/
		...
		void EraseVertex(const Vertex<ElemType>* toErase);
	...
	private:
		...
		*******************************************/
		...
		//Decreases the size of the adjacency matrix by one.
		void DecrementMatrix(int deletedIndex);
 ...
 *************************************************************************/
template <typename ElemType>
const Vertex<ElemType>* Graph<ElemType>::GetVertex(const ElemType& key) const
{
 ...
 * -----------------------------------------------------------------------
 * Returns the weight of the edge between the two vertecis containing
 * the provided elements. If no edge exists it returns -1.
 * -----------------------------------------------------------------------
 * PRE-CONDITIONS
 * 	start : The element stored in the first vertex.
 * 	stop  : The element stored in the second vertex.
 *
 * POST-CONDITIONS
 * 	weight : The weight of the edge between the first and second vertex.
 *************************************************************************/
template <typename ElemType>
float Graph<ElemType>::GetDistance(const ElemType& start, const ElemType& stop) const
{
	//VARIABLE DECLERATIONS
	const Vertex<ElemType>* vertOne;//PROC - Stores the start element.
	const Vertex<ElemType>* vertTwo;//PROC - Stores the second element.
	float weight;                   //OUT  - Weight of the edge. Equal to
	                                //       -1 if none exists.

	//Find the vertices containing the provided elements.
	vertOne = GetVertex(start);
	vertTwo = GetVertex(stop);

	//Check the adjacencyMatrix for an edge connecting them and if it exists
	//then find its weight.
	if(adjacencyMatrix[vertOne->GetPosition()][vertTwo->GetPosition()] != NULL)
	{
		weight = adjacencyMatrix[vertOne->GetPosition()][vertTwo->GetPosition()]->GetWeight();
	}
	else
	{
		weight = -1;
...
{
	//VARIABLE DECLERATIONS
	//PROC - Stores the starting point for the search for each individual vertex.
	const Vertex<ElemType>* origin;

	//PROC - Used to removed duplicate destinations.
	Stack<const Vertex<ElemType>* > tempStack;

	//PROC - Used to remove duplicate destinations
	typename List<const Vertex<ElemType>* >::Iterator destIt;
		                                          ...
		                                          Stack<const Edge<ElemType>* >&   edgeStack) const
{
	...
	while(findNextDest)
	{
		 ...
		 *****************************************************************/
//		cout << "START: " << start->GetElem().get_stadium_name() << endl;
//		cout << "ACTIVE LIST: " << endl;
//		while(activeIt != activeVertices.End())
//		{
//			cout << (*activeIt)->GetElem().get_stadium_name();
//			cin.ignore(1000, '\n');
//			++activeIt;
//		}
//		cout << endl;
...
void Graph<ElemType>::EraseVertex(const Vertex<ElemType>* toErase)
{
	...
	if(vertexList.Search(*toErase) != vertexList.End())
	{
		...
		for(int i = 0; i < size; ++i)
		{
			...
			delete adjacencyMatrix[i][toErase->GetPosition()];

			adjacencyMatrix[toErase->GetPosition()][i] = NULL;
...
void Graph<ElemType>::DecrementMatrix(int deletedIndex)
{
	...
	{
		for (int j = 0; j < deletedIndex; ++j)
		{
			matrix[i][j] = adjacencyMatrix[i][j];
		}

		for (int k = deletedIndex + 1; k < size; ++k)
		{
			matrix[i][k] = adjacencyMatrix[i][k];
		}
	}

	for(int i = deletedIndex + 1; i < size; ++i)
	{
		for (int j = 0; j < deletedIndex; ++j)
		{
			matrix[i][j] = adjacencyMatrix[i][j];
